package com.yichu.lucene.demo.search;
...
public class QueryParserDemo {
	...
	public static void main(String[] args)
		...
		// 索引存储目录
		Directory directory = FSDirectory.open(Paths.get("c:/indextest"));
		// 索引读取器
		...
		// 通过parse解析输入，生成query对象    price 去掉就可以搜索到了？？？ 难道和 MyIntField有关系
		...
		// 为什么query3查不出结果？？？ 该如何改   因为数值的查询，不能通过解析器来解析生成，只能处理文本的查询
		BooleanQuery bquery = new BooleanQuery.Builder()
				.add(parser.parse("(\"联想笔记本电脑\" OR simpleIntro:英特尔) AND type:电脑 "), Occur.MUST)
				.add(IntPoint.newRangeQuery("price", 800000, 1000000), Occur.MUST)
				.build();
